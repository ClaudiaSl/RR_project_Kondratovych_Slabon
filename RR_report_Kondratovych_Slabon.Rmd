...
# Selecting penultimate date of the unemployment analysis in OECD countries. Choosing penultimate date can prevent from missing data in some countries.
...
# Conducting analysis between 14 months before newest data and penultimate month.
plot_unemp_previous <- data_unemp %>%
  filter(TIME == nth(unemp_analysis_date, -14) & Country != "G-7" & Country != "EU28" & Country != "EA19" & Country != "EU27_2020")%>%
  ...
  theme_light() +
  coord_flip() +
  labs(title=paste0("Highest unemployment rate in ",nth(unemp_analysis_date, -14)))+
  theme(
...
plot_unemp_current <- 
 data_unemp %>%
  filter(TIME == nth(unemp_analysis_date, -2) & Country != "G-7" & Country != "EU28" & Country != "EA19" & Country != "EU27_2020" & Country != "OECD")%>%
  select(Country, Value) %>%
  arrange(desc(Value)) %>%
  top_n(15) %>%
  arrange(Value) %>%
  ...
  theme_light() +
  coord_flip() +
  labs(title=paste0("Highest unemployment rate in ",nth(unemp_analysis_date, -2)))+
  theme(
    panel.grid.major.y = element_blank(),
    panel.border = element_blank(),
    axis.ticks.y = element_blank(),
...
plot_unem_dates_diff <- data_unemp %>%
  filter(TIME == nth(unemp_analysis_date, -14) | TIME == nth(unemp_analysis_date, -2)) %>%
  ...
  labs(title = "The greatest change in unemployment rate between newer and older chosen date",
